...
from PiyushMusic import YouTube, app
from PiyushMusic.core.call import Piyush
from PiyushMusic.misc import SUDOERS, db
from PiyushMusic.utils.database import (is_active_chat,
...
from PiyushMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from PiyushMusic.utils.stream.autoclear import auto_clean
from PiyushMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Piyush.pause_stream(chat_id)
        ...
        await Piyush.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Piyush.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Piyush.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Piyush.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Piyush.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Piyush.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
